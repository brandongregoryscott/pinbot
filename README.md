...
- `battle` — puts two random pins head to head
- `help` — lists all commands and aliases

## Pin Battles

Start a battle with `@Pinbot battle`. Pinbot presents two distinct random pins and adds 🅰️ and 🅱️ reactions. Each human participant has one vote and can change it by reacting to the other option. A battle ends as soon as three unique people have voted, or after one hour. Timed-out ties are recorded as draws; Pinbot's joke tiebreak choice does not change either pin's totals.
